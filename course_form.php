...
class course_status_form extends moodleform {
    ...
    public function display_report() {
        ...
        $columns = array('s_no' => get_string('s_no', 'block_course_status_tracker'),
            ...
            /* WWL	'grade' => get_string('grade', 'block_course_status_tracker'),*/
        );
        $hcolumns = array();
        ...
        $table->head = array(get_string('s_no', 'block_course_status_tracker'), get_string('course_name', 'block_course_status_tracker'), get_string('course_comp_date', 'block_course_status_tracker') /*, WWL-remove this column get_string('grade', 'block_course_status_tracker')*/);
		...
		foreach ($rs as $log) {
            $row = array();
            $row[] = ++$i;
            $row[] = "<a href=" . $CFG->wwwroot . "/course/view.php?id=" . $log->course . ">" . course_name($log->course) . "</a>";
			/*WWL original - course_name($log->course); */
			 
			
            $row[] = userdate($log->dates, get_string('strftimedate', 'core_langconfig'));
            /*WWL-remove grade column $row[] = round($log->gradefinal) . '%';*/
            $table->data[] = $row;
        }
        return $table;
    }
